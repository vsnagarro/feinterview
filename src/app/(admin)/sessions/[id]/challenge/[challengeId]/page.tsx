...
export default async function AdminLiveViewPage({
  params,
}: {
    ...
    <div className="p-6 flex flex-col h-full min-h-screen">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <h1 className="text-xl font-bold text-slate-900">
            Live Code View — {challenge.title}
          </h1>
        </div>
        <div className="flex items-center gap-2">
          {expired ? (
            ...
            <Badge variant="success">Active — {timeUntil(link.expires_at)} left</Badge>
          )}
          {link.opened_at ? (
            <Badge variant="info">Candidate connected</Badge>
          ) : (
            <Badge variant="default">Waiting for candidate</Badge>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0">
        <AdminCodeViewer
          ...
          candidateName={link.candidate_name ?? candidateName}
        />
      </div>
    </div>
  )
