...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      ...
      .select('id, created_at, difficulty, status, candidate_id')
  ...
  const candidateMap = new Map<string, string>(
    (allCandidates as CandidateRow[] | null)?.map((c) => [c.id, c.name]) ?? []
  )
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="card">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <Link href="/sessions" className="text-sm text-sky-600 hover:underline">View all</Link>
        </div>
        {recentSessions && recentSessions.length > 0 ? (
              ...
              >
                <div>
                  <p className="font-medium text-slate-900 text-sm">
                    {candidateMap.get(session.candidate_id) ?? 'Unknown candidate'}
                  </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded capitalize ${
                    ...
                    session.status === 'completed' ? 'bg-sky-100 text-sky-700' :
          ...
          <div className="p-8 text-center text-slate-400 text-sm">
            No sessions yet. <Link href="/sessions/new" className="text-sky-600 hover:underline">Create your first session</Link>
          </div>
        )}
      </div>
    </div>
