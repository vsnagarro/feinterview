...
export default function ChallengeClient({ token }: Props) {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const { data, error } = await supabase.from("challenges").select("*").eq("share_token", token).limit(1);
      ...
      if (ch && ch.id) {
        ...
        const { data: subs } = await supabase.from("submissions").select("*").eq("challenge_id", ch.id).order("created_at", { ascending: true });
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <select title="Language" value={language} onChange={(e) => setLanguage(e.target.value)} className="mt-1 p-2 border rounded">
